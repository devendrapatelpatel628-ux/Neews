...
# --- Bot Credentials ---
TOKEN = os.environ.get('BOT_TOKEN', '8678049860:AAF8k-oXG1HhC-rZQa6ozyReGqKFDzbVSQs')
...
def get_script_folder(user_id, file_name):
    """Get isolated folder for each script (auto-detect)"""
    user_path = os.path.join(UPLOAD_BOTS_DIR, str(user_id))
    os.makedirs(user_path, exist_ok=True)
    
    # First try: script-name folder
    script_name = os.path.splitext(file_name)[0]
    script_name = re.sub(r'[^\w\-_]', '_', script_name)
    direct_folder = os.path.join(user_path, script_name)
    direct_file = os.path.join(direct_folder, file_name)
    
    if os.path.exists(direct_file):
        return direct_folder
    
    # Second try: search all subfolders for the file
    if os.path.exists(user_path):
        for subdir in os.listdir(user_path):
            sub_path = os.path.join(user_path, subdir)
            if os.path.isdir(sub_path):
                if os.path.exists(os.path.join(sub_path, file_name)):
                    return sub_path
    
    # Third try: file is in user root (old structure)
    if os.path.exists(os.path.join(user_path, file_name)):
        return user_path
...
def process_python_install(message, requesting_user_id):
    ...
    # Validate package names (supports version specifiers)
    invalid = []
    for pkg in packages:
        if not re.match(
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    ver_result = subprocess.run(
                        [sys.executable, '-m', 'pip', 'show', pkg_base],
                        capture_output=True,
                        text=True,
                        encoding='utf-8',
                        errors='ignore'
...
def process_npm_install(message, requesting_user_id):
    ...
    # Validate package names (supports versions and scoped)
    invalid = []
    for pkg in packages:
        if not re.match(
            r'^[@a-zA-Z0-9_\-\.\/]+(@[\d\.\w\-]+)?$',
...
def process_uninstall(message, requesting_user_id):
    ...
    # Allow package name with extras like requests[security]
...
def handle_file_upload(message, file_content, file_name,
    ...
    """Handle single file upload with isolated folder"""
    user_id = message.from_user.id
    file_type = file_ext.replace('.', '')

    # ISOLATED FOLDER per script
    ...
    if user_id != OWNER_ID:
        is_susp, reason, severity = is_suspicious_file(
            file_content, file_name, file_ext.lower()
        )
        if is_susp:
            request_owner_approval(
                file_content, file_name, file_type,
                file_path, script_folder, reason,
                severity, message
            )
            return
    ...
    save_user_file(
        user_id, file_name, file_type, len(file_content)
    )

    # Run in OWN thread + OWN folder
    if file_type == 'py':
        threading.Thread(
            target=run_script,
            args=(file_path, user_id, script_folder,
                  file_name, message),
            daemon=True,
            name=f"Bot_{user_id}_{file_name}"
        ).start()
    elif file_type == 'js':
        threading.Thread(
            target=run_js_script,
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    """Handle ZIP with isolated folder per ZIP"""
    user_id = message.from_user.id
    temp_dir = None

    # ISOLATED FOLDER for this ZIP
    zip_base = os.path.splitext(file_name_zip)[0]
    zip_base = re.sub(r'[^\w\-_]', '_', zip_base)
    script_folder = os.path.join(
        UPLOAD_BOTS_DIR, str(user_id), zip_base
    )
    os.makedirs(script_folder, exist_ok=True)

    try:
        temp_dir = tempfile.mkdtemp(
            prefix=f"u{user_id}_", dir=TEMP_DIR
        )
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            if user_id != OWNER_ID:
                for m in zip_ref.infolist():
                    if any(m.filename.lower().endswith(ext)
                           for ext in EXECUTABLE_EXTENSIONS):
                        request_owner_approval(
                            downloaded_file_content,
                            file_name_zip, 'zip',
                            zip_path, script_folder,
                            f"ZIP has: {m.filename}",
                            "high", message
                        )
                        return
                    mp = os.path.abspath(
                        os.path.join(temp_dir, m.filename)
                    )
                    if not mp.startswith(
                            os.path.abspath(temp_dir)):
                        raise zipfile.BadZipFile(
                            f"Path traversal: {m.filename}"
                        )
            zip_ref.extractall(temp_dir)

        # Find script directory
        target = temp_dir
        if not any(f.endswith(('.py', '.js'))
        ...
        if 'requirements.txt' in items:
            req = os.path.join(temp_dir, 'requirements.txt')
            ...
            try:
                ...
                bot.reply_to(message, "✅ Requirements installed!")
            except Exception as e:
                logger.error(f"Req error: {e}")

        if 'package.json' in items:
            bot.reply_to(message, "🔄 Installing npm...")
            try:
                subprocess.run(
                    ['npm', 'install'],
                    capture_output=True, text=True,
        ...
        if not main_file:
            bot.reply_to(message, "❌ No script in ZIP!")
            return

        # Move ALL files to ISOLATED script folder
        for item in os.listdir(temp_dir):
            if item == file_name_zip:
                continue
            s = os.path.join(temp_dir, item)
            d = os.path.join(script_folder, item)
            if os.path.exists(d):
                if os.path.isdir(d):
                    shutil.rmtree(d)
                else:
        ...
        main_path = os.path.join(script_folder, main_file)
        bot.reply_to(
            message,
            f"✅ *ZIP Extracted!*\n"
            f"📁 Folder: `{zip_base}/`\n"
            f"📄 Main: `{main_file}`\n"
            f"⏳ Starting in isolated thread...",
            parse_mode='Markdown'
        )

        save_user_file(
            user_id, main_file, file_type,
        ...
        if file_type == 'py':
            threading.Thread(
                target=run_script,
                args=(main_path, user_id, script_folder,
                      main_file, message),
                daemon=True,
                name=f"ZIP_{user_id}_{main_file}"
            ).start()
        elif file_type == 'js':
            threading.Thread(
                target=run_js_script,
    ...
    def do_check():
        try:
            ...
            for uid, fname in db_files:
                folder = get_script_folder(uid, fname)
    ...
    def do_start_all():
        ...
        for uid, files in list(user_files.items()):
            for fname, ftype in files:
                ...
                user_folder = get_script_folder(uid, fname)
...
def handle_restore_version(call):
    ...
    try:
        ...
        user_folder = get_script_folder(owner_id, fname)
...
# Add this command temporarily
@bot.message_handler(commands=['debug'])
def debug_files(message):
    if message.from_user.id != OWNER_ID:
        return
    ...
    # Check upload folder
    user_path = os.path.join(UPLOAD_BOTS_DIR, str(user_id))
    output += f"User path: {user_path}\n"
    output += f"Exists: {os.path.exists(user_path)}\n\n"
...
def master_callback_handler(call):
    ...
    try:
        ...
        # === FILE CONTROL ===
            ...
            folder = get_script_folder(owner_id, fname)
            fpath = os.path.join(folder, fname)
            if not os.path.exists(fpath):
